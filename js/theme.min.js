!function() {
    ...
    e.addEventListener("change", (function() {
        this.checked && (o.style.display = "",
        n.style.display = "none")
    }
    )),
    t.addEventListener("change", (function() {
        this.checked && (n.style.display = "",
    ...
    document.querySelectorAll(".password-field input").forEach((function(e) {
        e.addEventListener("keyup", (function() {
            ...
            t.classList.add("password-field", "level" + e)
        }
        ))
    }
    ));
    ...
    const s = document.getElementById("invoice");
    if (s) {
        s.querySelector(".print-link").addEventListener("click", (function() {
            window.print()
        }
        ))
    }
    const i = document.querySelectorAll(".sidebar-nav-fixed a");
    i.forEach((function(e) {
        e.addEventListener("click", (function(e) {
            if (location.pathname.replace(/^\//, "") === this.pathname.replace(/^\//, "") && location.hostname === this.hostname) {
                var t = document.querySelector(this.hash);
                (t = t || document.querySelector("[name=" + this.hash.slice(1) + "]")) && (e.preventDefault(),
                window.scrollTo({
                    top: t.offsetTop - 90,
                    behavior: "smooth"
                }),
                t.focus(),
                t !== document.activeElement && (t.setAttribute("tabindex", "-1"),
                t.focus()))
            }
            i.forEach((function(e) {
    ...
    d && new Tagify(d);
    const u = document.querySelector(".headingTyped");
    if (u)
        new Typed(u,{
    ...
    const m = document.getElementById("liveAlertPlaceholder")
      , p = document.getElementById("liveAlertBtn");
    m && p && p.addEventListener("click", (function() {
        !function(e, t) {
            const n = document.createElement("div");
            n.innerHTML = '<div class="alert alert-' + t + ' alert-dismissible" role="alert">' + e + '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>',
            m.appendChild(n)
        }("Nice, you triggered this alert message!", "success")
    }
    ));
    const f = document.getElementById("typed");
    if (f)
        new Typed(f,{
    ...
    const h = document.getElementById("slider");
    ...
    var y = document.querySelector(".btn-scroll-top");
    if (null != y) {
        var g = document.querySelector(".btn-scroll-top path")
          , v = g.getTotalLength();
        g.style.transition = g.style.WebkitTransition = "none",
        g.style.strokeDasharray = v + " " + v,
        g.style.strokeDashoffset = v,
        g.getBoundingClientRect(),
        g.style.transition = g.style.WebkitTransition = "stroke-dashoffset 10ms linear",
        window.addEventListener("scroll", (function(e) {
            var t = document.body.scrollTop || document.documentElement.scrollTop
              , n = document.documentElement.scrollHeight - document.documentElement.clientHeight
              , o = v - t * v / n;
            g.style.strokeDashoffset = o,
            (document.body.scrollTop || document.documentElement.scrollTop) >= 50 ? y.classList.add("active-progress") : y.classList.remove("active-progress")
        }
        )),
...
if (changelogElement) {
    function changeLog() {
        changelogElement.innerHTML = "v3.1.1"
    }
    changeLog()
}
